...
class RealtimeTrade:
    ...
    def __init__(self, prod):
        ...
        self.sell_sell_price_from_stop_price = 0.99
        self.check_init()
    ...
    def refresh_markets(self):
        ...
        try:
            ...
            print(self.exchange_name, f'Create order failed:', type(e).__name__, str(e))
    ...
    def _add_order_to_db(self, symbol, order_details):
        ...
        details = dict(is_prod=self.is_production, order_dt=order_dt, exchange=self.exchange_name, trade_id=trade_id,
                       type=type, side=side, status=status,
                       symbol=symbol, price=price, stopPrice=stopPrice, amount_requested=amount_requested,
    ...
    def _create_buy_market(self, coin, buy_price, stop_loss_price):
        ...
        try:
            self.check_algo_price(symbol, buy_price)
            ...
            print(
                f'Market Buy: {symbol} - price(ticker)={f_curr_price}, amount={f_amount},'
            ...
            print(self.exchange_name, symbol, f'Create MARKET BUY order failed:', type(e).__name__, str(e))
    ...
    def _create_stop_loss_request(self, coin, stop_price, order_details):
        ...
        sell_price = stop_price * self.sell_sell_price_from_stop_price
        f_amount = self.exchange.amount_to_precision(symbol, amount)
        ...
        try:
            ...
            if not use_locked:
                ...
                raise ValueError(f'Trying to create stoploss order when there is amount locked for {coin}')
        except Exception as e:
            print(self.exchange_name, symbol, f'Create STOP_LOSS order failed:', type(e).__name__, str(e),
                  f'details: f_amount={f_amount} f_stop_price={f_stop_price}, f_sell_price{f_sell_price}')

    def _check_buy_and_price(self, coin):
        use_locked = False
        symbol = f'{coin}/{self.stable_coin_name}'
        amount_holding, amount_locked = self.get_asset_holding_amount(self.stable_coin_name)
        ...
        # TODO check if locked is needed, as the usdt will never be on an open order.
    ...
    def _check_sell_and_price(self, coin):
        ...
        use_locked = False
        symbol = f'{coin}/{self.stable_coin_name}'
        amount, amount_locked = self.get_asset_holding_amount(coin)
        curr_price = self.get_current_price(symbol)
        amount_stable = amount * curr_price  # convert to stable coin
        is_enough_funds = amount_stable > self.stable_coin_min_amount
        amount_locked_stable = amount_locked * curr_price  # try with locked
        if not is_enough_funds and amount_locked_stable > self.stable_coin_min_amount:
            amount = amount_locked
            amount_stable = amount_locked_stable
            is_enough_funds = True
            use_locked = True
        if not is_enough_funds:
    ...
    def check_algo_price(self, symbol, req_price):
        curr_price = self.get_current_price(symbol)
        diff_pct = (req_price / curr_price) - 1
        if diff_pct > self.price_diff_pct:  # 0.09 > 0.05
            print(f'Warning: {symbol} algo price is *LARGER* than {int(self.price_diff_pct * 100)}%,'
                  f' sell_price={req_price}, curr_price={curr_price}, diff={diff_pct:.2%}')
        elif diff_pct < -self.price_diff_pct:  # -0.09 < -0.05
            print(f'Warning: {symbol} algo price is **LOWER** than {int(self.price_diff_pct * 100)}%,'
                  f' sell_price={req_price}, curr_price={curr_price}, diff={diff_pct:.2%}')
    ...
    def _create_sell_market(self, coin, sell_price):
        ...
        symbol = f'{coin}/{self.stable_coin_name}'
        try:
            self.check_algo_price(symbol, sell_price)
            curr_price, amount_holding, amount_stable, use_locked = self._check_sell_and_price(coin)
            if use_locked:
                self._unlock_symbol(symbol)
                curr_price, amount_holding, amount_stable, _ = self._check_sell_and_price(
                    coin)  # TODO: Maybe a solution when multiple unlock were performed.
            ...
            print(self.exchange_name, symbol, f'Create MARKET SELL order failed:', type(e).__name__, str(e))

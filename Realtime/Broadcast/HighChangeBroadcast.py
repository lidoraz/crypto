...
def get_broadcast_buy_sell(result, strategy):
    ...
    for res in result['buy']:
        buys_txt += f"{res['coin']} {format_num(res['buy_price'])}, ({format_num(res['sell_price_lose_stop'])}, {format_num(res['sell_price_win_stop'])})\n"
    nl = ""
    if len(buys_txt):
    ...
    for res in result['sell']:
        sells_txt += f"{res['coin']} {format_num(res['sell_price'])}\n"
    if len(sells_txt):

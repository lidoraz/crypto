...
from .traces import *
from Indicators import SMA, RSI, MACD, BollingerBands, CandleIdentification
...
def get_updated_fig(providers, start_datetime, resample_keyword, coin, lookahead=25, xy_limit=True):
    ...
    fig.add_trace(get_candlestick(df_ohlc), row=1, col=1)

    ind_ptrn = CandleIdentification()
    ind_ptrn.calc(df_ohlc)
    ind_ptrn.plot(fig, loc=(2, 1))

...
def create_html(df):
    ...
    with open('daily_insider.html', 'w', encoding="utf-8") as f:
        ...
        f.write(
            f"""<h6> from openinsider.com, Updated to: {datetime.now(tz=None).strftime('%a, %B %d, %Y at %H:%M UTC')}<a style="color:White;" href="daily_swing.html">Swing✓</a></h6>""")
...
def get_ticker_img(ticker):
    path = icon_url.format(ticker.upper())
    # <a  target=_blank href="http://openinsider.com/{x}">{x}</a>
    html_img = f"""<div class="cont-img"><img src="{path}" loading="lazy" class="ticker-img"/> </div>"""
    return html_img
